...
- Email sign-in uses a one-time code sent through SMTP. Only complete email
  ...
  `stratc.org`. The address is checked before sending a code and when Better
...
- Any user who successfully signs in with the configured Mattermost OAuth
  application gets a regular IP Intel account. Mattermost sign-in is hidden
...
| `AUTH_EMAIL_DOMAINS` | Comma-separated exact email domains; defaults to `stratc.org`. |
...
do not put it in a `VITE_*` variable.
